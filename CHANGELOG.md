...
- **docs**: The setup writes a global `~/.justfile` with ~18 machine-wide recipes (`flush-dns`, `docker-clean`, `ports`, `standup`, `loc`, `ip`, `ds-clean`, …), but nothing told you how to run one — and plain `just` cannot find it: it searches upward from the current directory, so anywhere outside `$HOME` it reports `error: no justfile found`. The recipes are only reachable through `just -g` / `--global-justfile`, a flag that appeared in no generated doc; `~/.justfile` was named exactly twice in the whole script, once in the end-of-run config list and once in the `--uninstall` removal list, neither showing an invocation. So ~18 recipes sat on disk permanently unusable unless you already knew the flag — the mirror image of #237/#238, where a doc promised a command that did not exist; here the command exists and the invocation was missing. The `just` entry in `TOOL_REFERENCE` now documents the global file and `-g`, `POST_SETUP_CHECKLIST` gains a two-minute "run `jg --list` once to see what is there" item, the end-of-run config line says how to run them, and a **`jg`** alias (`just -g`) is added to the interactive alias block and to `docs/SHORTCUTS.md`. Every recipe name quoted in the new docs was cross-checked against the `JUSTFILE_CONF` heredoc, and `-g`/`--global-justfile` against `just --help` (#273, closes #271)

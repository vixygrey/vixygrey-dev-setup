# Windows Keyboard Shortcuts & Aliases

Quick reference for all shortcuts and aliases configured by the setup scripts on Windows.

---

## Shell Aliases

> **Note:** PowerShell uses `Set-Alias` for simple command mappings and PowerShell functions for commands with arguments. The setup script configures these in your `$PROFILE`. The aliases below match the bash/zsh aliases but are implemented as PowerShell equivalents.
...
| Alias | Runs | What it does |
|-------|------|-------------|
| `update` | `topgrade` | Update everything (scoop, npm, pip, OS) |
...
These PowerShell-specific aliases cover common commands that differ from their bash equivalents:

| Alias/Function | Command | Bash equivalent |
|----------------|---------|-----------------|
| `ls` | `eza --icons` | Same |
| `ll` | `eza -la --icons --git` | Same |
| `cat` | `bat --paging=never` | Same |
| `psg` | `Get-Process \| Where-Object...` | `procs` |
| `which` | `Get-Command` | `command -v` |
| `touch` | `New-Item` | `touch` |

> **Note:** PowerShell aliases only support single-command mappings. For commands with arguments, the script creates PowerShell functions instead.
...
|----------|-------|
| Shell aliases | 65 |
| PowerShell-specific aliases | 6 |
| VS Code keybindings | 21 |
| Terminal multiplexer keybindings | 12 |
| Vim keybindings | 8 |
| fzf keybindings | 7 |
| Git aliases | 30 |
...
| Windows Terminal shortcuts | 6 |
| Windows desktop shortcuts | 7 |
| **Total** | **224** |

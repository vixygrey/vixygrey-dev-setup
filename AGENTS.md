...
Config files, the OMP agent environment, the pre-commit hook, and the Desktop documents are generated by the script. Edit the generator, not an installed output.
...
`~/.zshrc` is sourced by non-interactive shells, so its content reaches automation agents and scripts. Aliases are an interactive convenience and **must be gated**. Gate the whole section on `[[ -o interactive && -z "$CLAUDECODE" && -z "$AI_AGENT" ]]`.
...
- **Skills:** `api-testing`, `d2-diagrams`, and `office-layout-check` under `~/.agents/skills/`.
